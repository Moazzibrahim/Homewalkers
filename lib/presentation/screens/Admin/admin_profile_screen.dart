...
class AdminProfileScreen extends StatelessWidget {
  ...
  Future<String> checkAuthEmail() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getString('email') ?? 'sales@gmail.com';
  }
  ...
  Widget build(BuildContext context) {
    ...
    final Color surface =
        isDark ? const Color(0xFF1E1E2E) : const Color(0xFFF0F2F8);
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          _buildStatusCard(context, main, surface),
          ...
          const SizedBox(height: 10),
          _buildConnectivityCard(context, main, cardBg, surface),
          const SizedBox(height: 14),
          // ── Interface ──────────────────────────────────────
          ...
          const SizedBox(height: 14),
          // ── Sign Out ───────────────────────────────────────
          _buildSignOutButton(context, Color(0xffBA1A1A33)),
  ...
  // ── STATUS CARD ──────────────────────────────────────────────
  Widget _buildStatusCard(BuildContext context, Color main, Color surface) {
    return Container(
      child: ListTile(
  ...
  // ── SIGN OUT BUTTON ──────────────────────────────────────────
  Widget _buildSignOutButton(BuildContext context, Color main) {
    return SizedBox(
      width: double.infinity,
      child: OutlinedButton.icon(
        onPressed: () => context.read<AuthCubit>().logout(context),
        icon: Icon(Icons.logout_outlined, color: Color(0xffBA1A1A), size: 18),
        label: Text(
          "SIGN OUT",
          style: TextStyle(
  ...
  // ── SHARED HELPERS ───────────────────────────────────────────
  BoxDecoration _cardDecoration(Color cardBg) => BoxDecoration(
    color: cardBg,
    borderRadius: BorderRadius.circular(16),
    boxShadow: [
      BoxShadow(
