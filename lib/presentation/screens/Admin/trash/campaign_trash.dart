...
class CampaignTrash extends StatefulWidget {
  const CampaignTrash({super.key});

  @override
  State<CampaignTrash> createState() => _CampaignTrashState();
}

class _CampaignTrashState extends State<CampaignTrash> {
                  ...
                  child: BlocBuilder<GetCampaignsCubit, GetCampaignsState>(
                    builder: (context, state) {
                      if (state is GetCampaignsLoading) {
                        ...
                        final all = state.campaigns.data ?? [];
                        final filtered =
                            all.where((c) {
                              if (_searchQuery.isEmpty) return true;
                              return (c.campainName ?? '')
                                  .toLowerCase()
                        ...
                        return ListView.separated(
                          ...
                          itemBuilder:
                              (context, index) =>
                                  _buildCampaignCard(filtered[index], context),
                        );
  ...
  // CAMPAIGN CARD
  // ─────────────────────────────────────────────────────────────────────────
  Widget _buildCampaignCard(CampaignData campaign, BuildContext context) {
    final isLight = Theme.of(context).brightness == Brightness.light;
    final mainColor =
        isLight ? Constants.maincolor : Constants.mainDarkmodecolor;
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Icon(Icons.attach_money, size: 14, color: mainColor),
                      ...
                      content: RichText(
                        text: TextSpan(
                          ...
                          children: [
                            ...
                            TextSpan(
                              text: campaign.campainName ?? 'this campaign',
                              style: TextStyle(
                                fontWeight: FontWeight.bold,
                                color: isLight ? Colors.black87 : Colors.white,
                              ),
