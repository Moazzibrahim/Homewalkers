...
class _ManagerLeadsScreenState extends State<AdminLeadsScreen> {
  ...
  Widget getStatusIcon(String status) {
    switch (status) {
      ...
      case 'Meeting':
        return Icon(Icons.chat_bubble_outline, color: Theme.of(context).brightness == Brightness.light ? Constants.maincolor : Constants.mainDarkmodecolor);
      case 'Done Deal':
        return Icon(Icons.check_box_outlined, color: Theme.of(context).brightness == Brightness.light ? Constants.maincolor : Constants.mainDarkmodecolor);
      case 'Interested':
        return Icon(FontAwesomeIcons.check, color: Theme.of(context).brightness == Brightness.light ? Constants.maincolor : Constants.mainDarkmodecolor);
      case 'Not Interested':
        return Icon(FontAwesomeIcons.timesCircle, color: Theme.of(context).brightness == Brightness.light ? Constants.maincolor : Constants.mainDarkmodecolor);
      case 'Fresh':
        return Icon(Icons.new_releases, color: Theme.of(context).brightness == Brightness.light ? Constants.maincolor : Constants.mainDarkmodecolor);
      case 'Transfer':
        return Icon(Icons.no_transfer, color: Theme.of(context).brightness == Brightness.light ? Constants.maincolor : Constants.mainDarkmodecolor);
      default:
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: CustomAppBar(
        title: 'Leads',
        onBack: () {
          Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => AdminTabsScreen()));
        },
      ),
      body: Padding(
              ...
              child: Row(
                children: [
                  Expanded(
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        hintText: 'Search',
                        hintStyle: GoogleFonts.montserrat(color: const Color(0xff969696), fontSize: 12, fontWeight: FontWeight.w500),
                        prefixIcon: Icon(Icons.search, color: Theme.of(context).brightness == Brightness.light ? Constants.maincolor : Constants.mainDarkmodecolor),
                        enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: Theme.of(context).brightness == Brightness.light ? Constants.maincolor : Constants.mainDarkmodecolor), borderRadius: BorderRadius.circular(8)),
                        focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: Theme.of(context).brightness == Brightness.light ? Constants.maincolor : Constants.mainDarkmodecolor), borderRadius: BorderRadius.circular(8)),
                  ...
                  Container(
                    height: 50.h,
                    width: 50.w,
                    decoration: BoxDecoration(color: const Color(0xFFE8F1F2), border: Border.all(color: Theme.of(context).brightness == Brightness.light ? Constants.maincolor : Constants.mainDarkmodecolor), borderRadius: BorderRadius.circular(8)),
                    child: IconButton(
                      icon: Icon(Icons.filter_list, color: Theme.of(context).brightness == Brightness.light ? Constants.maincolor : Constants.mainDarkmodecolor),
                      onPressed: () async {
                        if (selectedTab == 1) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text("Filtering is not available for the trash.")),
                          );
                          return;
                        }
                          ...
                          builder: (dialogContext) {
                            return MultiBlocProvider(
                              providers: [
                                ...
                                BlocProvider(create: (_) => ProjectsCubit(ProjectsApiService())..fetchProjects()),
                                BlocProvider(create: (_) => StagesCubit(StagesApiService())..fetchStages()),
                                BlocProvider(create: (_) => ChannelCubit(GetChannelsApiService())..fetchChannels()),
                              ...
                              child: FilterDialog(
                                ...
                                initialCommunicationWay: _selectedCommunicationWayFilter,
                        ...
                        if (filters != null) {
                          setState(() {
                            ...
                            _selectedCommunicationWayFilter = filters['communicationWay'];
                ...
                Row(
                  children: [
                    GestureDetector(
                      ...
                      child: Column(
                        children: [
                          Text('Manage Leads', style: GoogleFonts.montserrat(fontSize: 14, fontWeight: FontWeight.w600, color: selectedTab == 0 ? Constants.maincolor : Colors.grey)),
                          const SizedBox(height: 4),
                          if (selectedTab == 0) Container(height: 2, width: 50, color: Constants.maincolor),
                        ],
                      ),
                    ),
                    ...
                    GestureDetector(
                      ...
                      child: Column(
                        children: [
                          Text('Leads Trash', style: GoogleFonts.montserrat(fontSize: 14, fontWeight: FontWeight.w600, color: selectedTab == 1 ? Constants.maincolor : Colors.grey)),
                          const SizedBox(height: 4),
                          if (selectedTab == 1) Container(height: 2, width: 50, color: Constants.maincolor),
                        ],
                      ),
                    ),
                  ],
            ...
            Expanded(
              child: BlocBuilder<GetAllUsersCubit, GetAllUsersState>(
                builder: (context, state) {
                  if (state is GetAllUsersLoading || state is GetLeadsInTrashLoading) {
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              subtitle: Text("Phone: ${lead.phone ?? 'N/A'}\nEmail: ${lead.email ?? 'N/A'}"),
