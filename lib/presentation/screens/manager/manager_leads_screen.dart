                        ...
                        child: Padding(
                            ...
                            child: BlocBuilder<LeadCommentsCubit, LeadCommentsState>(builder: (context, state) {
                              if (state is LeadCommentsLoading) {
                                ...
                                final firstcommentdate = DateTime.tryParse(firstComment?.firstcomment?.date.toString() ?? "")?.toUtc();
                                final secondcommentdate = DateTime.tryParse(firstComment?.secondcomment?.date.toString() ?? "")?.toUtc();
                                final isFirstValid = isClearHistoryy != true || (firstcommentdate != null && firstcommentdate.isAfter(clearHistoryTimee!));
                                final isSecondValid = isClearHistoryy != true || (secondcommentdate != null && secondcommentdate.isAfter(clearHistoryTimee!));
                                if ((isFirstValid && firstComment?.firstcomment?.text != null)) {
                                  return Column(
                                    ...
                                    children: [
                                      ...
                                      const SizedBox(height: 5),
                                      Text(firstComment?.secondcomment?.text ?? 'No comment available.'),
                                    ],
                                  );
                                } else {
          ...
          child: InkWell(
            onTap: () async {
              ...
              await Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => LeadsDetailsScreenManager(
                    leedId: lead.id!,
                    leadName: lead.name ?? '',
